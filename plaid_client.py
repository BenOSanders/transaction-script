...
configuration = plaid.Configuration(
    host=ENV, api_key={"clientId": CLIENT_ID, "secret": PLAID_SECRET}
)
...
def exchange_public_token(public_token: str) -> tuple[str, str]:
    ...
    item = Item(item_id=res.item_id, access_token=res.access_token)
    insert_item(item)
    init_account(item)
    return res.item_id
...
def sync_plaid_transactions() -> dict:
    ...
    for item in items:
        ...
        data: TransactionsSyncResponse = client.transactions_sync(request)
        data = data.to_dict()

        added_tx: list = data["added"]
